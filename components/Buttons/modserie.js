...
const Modserie = ({ isVisible, onClose,selectedData,area_id, }) => {
    if (!isVisible) return null;
    const [values, setValues] = useState({
        area_id:area_id,
        id: selectedData ? selectedData.id : '',
        codigo: selectedData ? selectedData.codigo : '',
        serie: selectedData ? selectedData.serie : '',
        val_a: selectedData ? selectedData.val_a : '',
        val_l: selectedData ? selectedData.val_l : '',
        val_f: selectedData ? selectedData.val_f : '',
        vig_ac: selectedData ? selectedData.vig_ac : '',
        vig_at: selectedData ? selectedData.vig_at : '',
        total: selectedData ? selectedData.total : '',
        eliminacion: selectedData ? selectedData.eliminacion : false,
        conservacion: selectedData ? selectedData.conservacion : false,
        muestreo: selectedData ? selectedData.muestreo : false,
        observaciones: selectedData ? selectedData.observaciones : ''
        });
        console.log (values)
        ...
        const handleRadioChange = (event) => {
          ...
          const handleSubmmit = async (event) => {
            ...
            try {
            const response = await axios.put("http://127.0.0.1:8000/api/tableareas/"+values.id, values);
            if (response.status === 200) {
            Swal.fire("Success", "Serie actualizada correctamente!", "success");
            onClose();
            }
            } catch (error) {
            Swal.fire("Error", "Error al actualizar serie!", "error");
            console.log(values);
            }
            };            
                ...
                <div role="alert" class="container mx-auto w-11/12 md:w-2/3 max-w-lg">
                    <div class="relative py-8 px-5 md:px-10 bg-white shadow-md rounded border border-gray-400">
                        ...
                        <div class="flex items-center justify-start w-full">
                            <button class="focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-700 transition duration-150 ease-in-out hover:bg-indigo-600 bg-indigo-700 rounded text-white px-8 py-2 text-sm" onClick={handleSubmmit}>Submit</button>
